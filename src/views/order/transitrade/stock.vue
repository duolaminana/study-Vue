<template>
  <div class="app-container stock">
    <div class="edit-title">销售订单备货记录</div>
...
import { saleOrderDetail, typeNodeSaleOrder, stockRecord, addRecordFile, updateOrderFlowInfo, deleteOrderStockRecord } from '@/api/transitrade'
    ...
    saveRecord(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let obj1 = {
            currentCompletion: this.stockForm.completeDegree,
          ...
          this.stockForm.salesOrderBId = this.$route.query.id;
          this.stockForm.id = this.$route.query.nodeId;
          this.stockForm.workflowProcessBId = this.$route.query.workflowId;
          addRecordFile(this.stockForm).then(res => {
            this.confirmLoading = false;
            this.getRecordList();
