      ...
      <el-row>
        <el-col>
          <el-table
            ...
            <el-table-column align="center" width="100" label="确收金额" prop="receivedAmount"/>
      ...
      <el-row class="row-item">
        <el-form ref="checkForm" :model="checkForm" label-width="140px" size="small" :rules="checkFormRules">
          ...
          <el-form-item label="备注说明：" prop="orderRemarks">
            <el-input v-model="checkForm.orderRemarks" :autosize="{ minRows: 3, maxRows: 5}" type="textarea" size="small" style="width: 60%"/>
          </el-form-item>
          ...
          <el-form-item label="客户备注说明：" prop="customerRemarks">
...
import { saleOrderDetail, allNodeSaleOrder, stockRecord, addRecordFile, checkSaleOrderFlow } from '@/api/transitrade'
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.checkForm.attachmentName) {
            this.checkForm.salesOrderBId = this.$route.query.id;
            this.checkForm.id = this.$route.query.nodeId;
            this.checkForm.workflowProcessBId = this.$route.query.workflowId;
            addRecordFile(this.checkForm).then(res => {
          ...
          this.detailForm.workFlowid = this.$route.query.nodeId;
          this.detailForm.orderRemarks = this.checkForm.orderRemarks;
          this.detailForm.adjustAmount = this.checkForm.adjustAmount;
          this.detailForm.salesOrderProductsList = this.orderSkuList;
          checkSaleOrderFlow(this.detailForm).then(res => {
            this.confirmLoading = false;
            this.closePage()
          }).catch(err => {
            this.confirmLoading = false;
