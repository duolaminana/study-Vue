<template>
  <div class="app-container menu-page">
    <div class="topBtns">
      ...
      <el-button type="primary" size="small" @click="deleteClientMenu()">{{$t('common.delete')}}</el-button>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?$t('system.menu.addMenu'):$t('system.menu.editMenu')" width="500px" center :close-on-click-modal="false" @closed="clearData">
      <el-form ref="addClientMenu" :model="menuData" label-width="145px" label-position="left" size="small" :rules="menuDataRules">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="small" :loading="confirmLoading" @click="confirmAdd('addClientMenu', dialogType === 'add' ? 1 : 2)">
    ...
    <el-dialog :visible.sync="dialogVisible1" :title="$t('system.menu.menuView')" width="500px" center :close-on-click-modal="false">
      <el-tabs value="1">
        <el-tab-pane :label="$t('system.menu.menuDetail')" name="1">
          <el-form ref="addClientMenu" :model="menuDetail" label-width="145px" label-position="left" size="small">
            <el-form-item :label="$t('system.menu.parentMenu')">
              <el-cascader
                class="form-dialog-width"
...
<script>
import { getClientMenuList, addClientMenu, changeClientMenuStatus, editClientMenu, deleteClientMenu, getMenuClientRole, clientMenuMoveOne, clientMenuMoveMast } from '@/api/system'
...
export default {
  data() {
    return {
      ...
      menuDataRules: {
        ...
        menuTitleZh: [
          { required: true, message: 'Please fill in chinese menu name', trigger: 'blur' }
        ],
        menuSign: [
  ...
  methods: {
    ...
    confirmAdd(formName, type) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (type === 1) {
            addClientMenu(this.menuData).then(res => {
            ...
            editClientMenu(this.menuData).then(res => {
    ...
    detailDialog(row) {
      ...
      getMenuClientRole({ menuBid: row.bid }).then(res => {
    ...
    changeStatus(row) {
      ...
      changeClientMenuStatus(obj).then(res => {
    ...
    moveFirst(node, data) {
      ...
      this.canClick = true;
      clientMenuMoveMast({ id: data.id, type: 1 }).then(res => {
    ...
    moveLast(node, data) {
      ...
      this.canClick = true;
      clientMenuMoveMast({ id: data.id, type: 2 }).then(res => {
    ...
    moveComm(arr, curIndex, nextIndex, data, move) {
      this.canClick = true;
      clientMenuMoveOne({ id: data.id, type: move === 1 ? 1 : 2 }).then(res => {
