<template>
  <div class="app-container roleManagement">
    <div class="search-box">
      <el-form label-width="90px" inline size="small">
        ...
        <el-form-item :label="$t('system.role.status')">
          <el-select v-model="postData.status" clearable :placeholder="$t('system.role.all')" style="width: 100px">
    ...
    <el-table
      ...
      <el-table-column align="center" width="160" :label="$t('system.role.time')" prop="createTime">
        <template slot-scope="scope">
          {{scope.row.createTime | timeFmt}}
        </template>
      </el-table-column>

      <el-table-column align="center" width="100" :label="$t('system.role.status')" prop="roleStatus">
        <template slot-scope="scope">
          <el-tag :type="scope.row.roleStatus === 1 ? 'primary' : 'danger' " size="mini">{{ scope.row.roleStatus === 0 ?
            $t('system.role.active') : $t('system.role.inactive') }}
          </el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('system.role.operation')" width="265px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" class="export table-btn-width" @click="openDetail(row)">{{$t('system.role.view')}}</el-button>
          <el-button type="primary" size="mini" class="table-btn-width" @click="openDialog(row)">{{$t('system.role.editPermission')}}</el-button>
  ...
  <!-- 新增角色-->
     ...
     <el-dialog
      ...
      <el-table
        ...
        <el-table-column align="center" width="80px" :label="$t('system.role.status')" prop="accountStatus">
          <template slot-scope="scope">
            <el-tag :type="scope.row.accountStatus === 1 ? 'primary' : 'danger' " size="mini">{{ scope.row.accountStatus
              === 0 ?
    ...
    <el-dialog :visible.sync="detailModal" :title="$t('system.role.roleDetail')" width="600px" center>
      <el-form :model="detailData" label-width="100px" label-position="right" size="small" inline>
        ...
        <el-form-item :label="$t('system.role.status')">
          {{detailData.roleStatus === 1 ? $t('system.role.disable') : $t('system.role.disable')}}
        </el-form-item>
      </el-form>
      <el-tabs value="1">
            ...
            <el-table-column align="center" :label="$t('system.role.account')" prop="prop">
              <template slot-scope="scope">
                <el-tag :type="scope.row.accountStatus === 1 ? 'primary' : 'danger' " size="mini">
                  {{scope.row.accountStatus === 0 ? $t('system.role.disable') : $t('system.role.disable') }}
                </el-tag>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-tabs value="1">
        <el-tab-pane :label="$t('system.role.editMenu')" name="1">
          <el-form :model="detailData" label-width="100px" label-position="right" size="small" inline style="border-bottom: 1px solid #f4f4f4">
            ...
            <el-form-item :label="$t('system.role.status')">
              {{detailData.roleStatus === 1 ? $t('system.role.active') : $t('system.role.inactive')}}
            </el-form-item>
          </el-form>
          <div style="max-height: 500px;overflow: auto">
...
export default {
  ...
  methods: {
    ...
    getRoleUserList(roleBid) {
      ...
      getRoleUserList({
        roleBid: this.roleUserData.roleBid,
        pageIndex: 1,
        pageSize: 10
      }).then(res => {
