  ...
  <div class="app-container editInquiry">
    ...
    <el-form ref="editForm" :model="editForm" label-width="120px" size="small" :rules="editRules" class="edit-form" v-loading="loading">
        ...
        <el-col :span="10">
          <table class="form-table" border="0" cellspacing="0" cellpadding="0" style="width: 100%">
            <tr>
              ...
              <td width="60%">
                <el-select v-model="editForm.costomerBId" disabled size="mini" placeholder="请选择公司名称" class="edit-form-width input-center">
            ...
            <tr>
              <td>买方联系人</td>
              <td>
                <el-select v-model="editForm.buyerContact" disabled size="mini" placeholder="请选择购买人" class="edit-form-width input-center">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="交付期限" prop="dateOfDelivery">
            到款后<el-input v-model="editForm.dateOfDelivery" class="input-center" style="width: 200px"/>个工作日
          </el-form-item>
        </el-col>
        <!--<el-col>-->
      ...
      <el-table
        ...
        <el-table-column align="center" width="100" label="商品" prop="inqueryNo" show-overflow-tooltip>
          <template slot-scope="scope">
            {{scope.row.productName}}
          </template>
        </el-table-column>

        <el-table-column align="center" width="120" label="SKU描述" prop="attrValueList" show-overflow-tooltip/>
        ...
        <el-table-column align="center" width="120" label="报价量" prop="quotedQuantity" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-input v-model.number="scope.row.quotedQuantity" size="small" :min="0" class="input-center" @change="changeNumber(scope.row, scope.row.quotedQuantity)"/>
          </template>
        </el-table-column>

        <el-table-column align="center" width="100" label="先前报价" prop="lastQuotedPrice" show-overflow-tooltip/>

        <el-table-column align="center" width="120" label="报价单价" prop="quotedPrice">
          <template slot-scope="scope">
            <el-input v-model.number="scope.row.quotedPrice" size="small" :min="0" class="input-center" @change="changePrice(scope.row, scope.row.quotedPrice)"/>
          </template>
        </el-table-column>

        <el-table-column align="center" width="120" label="报价金额" prop="quoteMoney"/>
      ...
      <el-row>
        ...
        <!--<el-col>-->
          <!--<el-form-item label="询价备注" prop="remarks">-->
            <!--<el-input v-model="editForm.remarks" readonly :autosize="{ minRows: 4, maxRows: 6}" type="textarea" size="small" clearable style="width: 80%"/>-->
          <!--</el-form-item>-->
        <!--</el-col>-->
        <el-col>
...
import { detailQuotedPrice, saveQuotedPrice, updateQuotedPrice, detailInquiryPrice } from '@/api/inquiry'
export default {
  components: {
    Pagination
  },
  ...
  watch: {
    'editForm.costomerBId' (val, oldVal) {
      if (val && !oldVal) {
        ...
        this.editForm.buyerBId = obj.bid;
    ...
    getDetail() {
      if (this.$route.query.id && !this.$route.query.inquiryId) {
        detailQuotedPrice({ quotedPriceBId: this.$route.query.id }).then(res => {
          ...
          this.editForm.costomerBId = this.editForm.buyerBId;
