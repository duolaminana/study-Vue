<template>
  <div class="app-container supplierRecord">
    <div class="connectRecord">
      <div class="cusInfo">
        <div class="cus-grade">
          <div class="grade-img">
            <img v-if="gradeObj.badgeTemplatePath" :src="gradeObj.badgeTemplatePath" alt="">
            <svg-icon v-else icon-class="member" size="50" color="#376FF6"></svg-icon>
          </div>
          <div class="grade-text">{{gradeObj.levelName ? gradeObj.levelName : '普通客户'}}</div>
        </div>
        <div class="cus-detail">
          <h2 class="cus-company">{{supplierInfo.companyName}}</h2>
          <div class="cus-main">
            <div>供应商名称：{{supplierInfo.supplierName}}</div>
            <div>电话：{{supplierInfo.mobilePhone}}</div>
            <div>E-mail：{{supplierInfo.emailAddress}}</div>
            <div>国家：{{countryObj.chEnName}}</div>
            <div>客户状态：{{supplierInfo.transaStatus === 1 ? '已成交' : '未成交'}}</div>
            <div>所在领域：{{supplierInfo.ownField}}</div>
            <div>来源：{{supplierInfo.customerSource | sourceName}}</div>
            <div>登记日期：{{supplierInfo.createTime | timeFmt1}}</div>
          </div>
          <el-button class="close-btn" type="primary" size="small" @click="closePage">返回</el-button>
        </div>
      </div>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?'供应商沟通记录新增': '供应商沟通记录编辑'" center width="950px" @closed="clearData">
      <el-form ref="addCompany" :model="adsPosition" label-width="100px" label-position="right" size="small" inline :rules="adsPositionRules">
        ...
        <el-form-item label="沟通主题" prop="commTheme">
          <el-input v-model="adsPosition.commTheme" clearable placeholder="请输入沟通主题" style="width: 773px"/>
        </el-form-item>
        ...
        <!--<el-form-item label="沟通结果内容" prop="commResult">-->
          <!--&lt;!&ndash;<el-input v-model="adsPosition.commResult" clearable type="textarea" :rows="5" placeholder="这里后期用富文本编辑器" style="width: 773px"/>&ndash;&gt;-->
          <!--<tinymce v-model="adsPosition.commResult" style="width: 773px" />-->
        <!--</el-form-item>-->
    ...
    <el-dialog :visible.sync="detailModal" title="客户沟通记录详情" center width="950px" @closed="clearData">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="danger" size="small" @click="detailModal=false">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  deleteSupplierCommunicationApi,
  supplierDetailById,
  gradeList
} from '@/api/supplier';
export default {
  ...
  methods: {
    ...
    getSupplierInfo() {
      supplierDetailById({ bid: this.$route.query.id }).then(res => {
        this.supplierInfo = res.data;
        if (res.data.supplierGradeBId) {
          this.gradeObj = this.gradeList.filter(e => {
            return e.bid === res.data.supplierGradeBId
          })[0]
        }
    ...
    getDataList() {
      ...
      this.postData.supplierBId = this.$route.query.id;
      listSupplierCommunicationApi(this.postData).then(res => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (type === 1) {
            ...
            this.adsPosition.supplierBId = id;
    ...
    disableClick(row) {
      this.$confirm('此操作将删除该沟通记录, 是否继续?', '提示', {
        ...
        deleteSupplierCommunicationApi(obj).then(res => {
          if (res.status === 200) {
