<template>
  <div class="app-container standardProduct">
    ...
    <div class="topBtns">
      ...
      <el-button type="primary" size="small" @click="addStandardProduct()">{{$t('common.add')}}</el-button>
      <el-button type="primary" size="small" @click="deleteStandardProduct()">{{$t('common.delete')}}</el-button>
      <el-button type="primary" size="small" class="export" @click="exportStandardProduct()">{{$t('common.export')}}</el-button>
    </div>
    <el-table
      ref="checkTable"
    ...
    <el-dialog :visible.sync="dialogVisible" :title="$t('product.product.addstandard')" width="800px" center>
      ...
      <div style="margin: 10px 0">
        ...
        <Pagination :total="dialogForm.total" :page.sync="dialogForm.pageNum" :limit.sync="dialogForm.pageSize" @pagination="getProductStandardList" ></Pagination>
      </div>
      <div slot="footer">
...
import { getProductStandardList,
  getStandardProductList,
  getPublicCategory,
  searchStandardSkuList,
  updateStandardShelfSku,
  saveProductStandard,
  ...
  methods: {
    ...
    expandTable(row, rows) {
      ...
      if (expandFlag && !hasChilds) {
        searchStandardSkuList({ productBId: row.productBId }).then(res => {
    ...
    getProductStandardList() {
      ...
      getProductStandardList(obj).then(res => {
    ...
    deleteStandardProduct() {
      ...
      this.$confirm('此操作将删除该商品, 是否继续?', '提示', {
        ...
        deleteProductStandard(obj).then(res => {
          this.searchData();
    ...
    openShelf(val) {
      ...
      this.$confirm('是否确定' + text + '?', '提示', {
        ...
        updateStandardShelfSku({ isShelf: val, productSkuList: shelfSku }).then(res => {
          if (res.status === 200) {
    ...
    confirmAdd() {
      ...
      saveProductStandard({ productInfoList: this.selectList1 }).then(res => {
